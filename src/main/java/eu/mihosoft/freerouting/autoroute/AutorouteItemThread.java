...
public class AutorouteItemThread implements Runnable {
    Collection<Item>[] items_to_route;
    ...
    public AutorouteItemThread(BatchAutorouter p_batch_autorouter, Collection<Item>[] p_items_to_route, int p_pass_no, boolean p_with_screen_message, int p_threadId){
    ...
    public void run(){
        int itemsPerThread = (items_to_route.length + 4 - 1) / 4;
        int startIndex = itemsPerThread * this.threadId;
        int endIndex = Math.min(startIndex + itemsPerThread, items_to_route.length);

        for (int i = startIndex; i < endIndex; i++)
        {
            if(this.items_to_route[i] == null) continue;
            for(Item curr_item : this.items_to_route[i]){
                ...
                if (autoroute_item(curr_item, curr_item.get_net_no(0), ripped_item_list, pass_no))
                {
    ...
    private boolean autoroute_item(Item p_item, int p_route_net_no, SortedSet<Item> p_ripped_item_list, int p_ripup_pass_no)
    {
        try
        {
            ...
            AutorouteEngine.AutorouteResult autoroute_result = findAndRoute(this.batch_autorouter, p_route_net_no, time_limit, autoroute_control, route_start_set, route_dest_set, p_ripped_item_list);
    ...
    synchronized private static AutorouteEngine.AutorouteResult findAndRoute(BatchAutorouter batch_autorouter, int p_route_net_no, TimeLimit time_limit, AutorouteControl autoroute_control, Set<Item> route_start_set,
        ...
        return autoroute_engine.autoroute_connection(route_start_set, route_dest_set, autoroute_control,
                p_ripped_item_list);
    }
